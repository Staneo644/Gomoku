...
impl Message {
	...
	pub fn display_message(&self) {
		let rectangle_color = match self.message_type {
			MessageType::Error => Some(Color { r: (0.9), g: (0.16), b: (0.22), a: (0.8) }),
			MessageType::Info => Some(Color { r: (0.16), g: (0.9), b: (0.22), a: (0.8) }),
		};
		...
		draw_rectangle(	x - text_dimensions.width * 0.05,
						y - text_dimensions.height * 1.5,
						text_dimensions.width * 1.4, text_dimensions.height * 2.,
						rectangle_color.unwrap_or(Color { r: (0.9), g: (0.16), b: (0.22), a: (0.8) }));
