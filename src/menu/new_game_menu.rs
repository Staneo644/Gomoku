use crate::menu::menu::{Menu, MenuOption, MenuAction, MENU_HEIGHT, MENU_WIDTH, OPTION_HEIGHT, OPTION_WIDTH, MENU_START_POINT};
use crate::game::{Game, GameMode, GameVariant, GameState};
...
impl NewGameMenu {
	pub fn new() -> Self {
		...
		for mode in MODE_OPTIONS {
			let y_position = MENU_START_POINT.y * 0.66 + 100. + index as f32 * (OPTION_HEIGHT + 10.);
			...
			menu.mode_options.push(MenuOption::new(
				mode.to_string(),
				action,
				(Vec2::new(x_position, y_position), Vec2::new(x_position + OPTION_WIDTH, y_position + OPTION_HEIGHT))
			));
			index += 1;
		}
		index += 1;
	...
	pub fn draw(&self) {
		let menu_to_scale = Vec2::new(scale_to_resolution(MENU_WIDTH), scale_to_resolution(MENU_HEIGHT * 1.5));
		let menu_start = Vec2::new(scale_to_resolution(MENU_START_POINT.x), scale_to_resolution(MENU_START_POINT.y * 0.66));
		...
		draw_text("NEW GAME", screen_width() * 0.5 - text_dimensions.width / 2., screen_height() * 0.25, scale_to_resolution(40.), WHITE);
